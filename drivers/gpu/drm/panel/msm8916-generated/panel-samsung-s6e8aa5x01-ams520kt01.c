...
static inline int set_brightness_cmd(struct mipi_dsi_device *dsi, int brightness) {
	...
	dsi_dcs_write_seq(dsi,  MCS_TX_ACL_OPR_CTRL, 0x50);
	dsi_dcs_write_seq(dsi,  MCS_TX_ACL_CTRL, 0x02); /* ACL on */

	index = candela_to_elvss[brightness];
	mipi_dsi_dcs_write_buffer(dsi, seq_elvss[index], ELVSS_CMD_CNT);

	dsi_dcs_write_seq(dsi, 0xb0, 0x07); /* 0x07 : Temp1? */
	dsi_dcs_write_seq(dsi, MCS_TX_TSET, 0x14); /* Temp : 0x14(20) */

	dsi_dcs_write_seq(dsi, 0xb0, 0x15); /* 0x15 : Temp2? */
	dsi_dcs_write_seq(dsi, MCS_TX_TSET, 0x0a); /* Temp : Seems to be fixed to 0x0a, fetched from panel*/
	//dsi_dcs_write_seq(dsi, MCS_TX_TSET, index < 7? 0xa : index); /* Temp : candela <= 29? 0xa : elvss_value. Not sure why*/
	
	mipi_dsi_dcs_write_buffer(dsi, gamma_cmd[brightness], GAMMA_CMD_CNT);

	dsi_dcs_write_seq(dsi, MCS_TX_GAMMA_UPDATE, 0x03);
...
static int s6e8aa5x01_ams520kt01_set_brightness(struct backlight_device *bl_dev)
{
	...
	//KEY ON
	dsi_dcs_write_seq(dsi, MCS_TX_LEVEL_KEY, 0x5a, 0x5a);

	set_brightness_cmd(dsi, brightness);

	//KEY OFF
	dsi_dcs_write_seq(dsi, MCS_TX_LEVEL_KEY, 0xa5, 0xa5);
...
static int s6e8aa5x01_ams520kt01_on(struct s6e8aa5x01_ams520kt01 *ctx)
{
	...
	dsi_dcs_write_seq(dsi, MCS_TX_LEVEL_KEY, 0x5a, 0x5a);
	dsi_dcs_write_seq(dsi, MCS_TX_PCD_POLARITY, 0x4c);
	...
	//mipi_dsi_dcs_write_buffer(dsi, seq_elvss[index], ELVSS_CMD_CNT);
	//dsi_dcs_write_seq(dsi, MCS_TX_GAMMA,
	...
	//mipi_dsi_dcs_write_buffer(dsi, seq_aid[index], AID_CMD_CNT);
	//dsi_dcs_write_seq(dsi, MCS_TX_AID_CTRL, 0x04, 0xee);
	//dsi_dcs_write_seq(dsi, MCS_TX_ELVSS_CTRL, 0xac, 0x0f);
	//dsi_dcs_write_seq(dsi, MCS_TX_GAMMA_UPDATE, 0x03);
	set_brightness_cmd(dsi, brightness);

	dsi_dcs_write_seq(dsi, MCS_TX_PENTILE, 0xd8, 0xd8, 0x40);

	dsi_dcs_write_seq(dsi, 0xb0, 0x06); /* 0x06 : De_DIM? */
	dsi_dcs_write_seq(dsi, MCS_TX_TSET, 0xA8);
	dsi_dcs_write_seq(dsi, MCS_TX_LEVEL_KEY, 0xa5, 0xa5);
